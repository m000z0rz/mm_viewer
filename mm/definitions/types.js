...
function Type(typeName, fields, size, basicReadFunction)
{
	...
	if (typeName.indexOf("[") !== -1)
	{
		...
		this.arrayOfTypeName = typeName.substr(open + 1, close) + typeName.substr(close + 1);
		this.arrayLength = parseInt(typeName.substr(open + 1, close));
		console.log("im an array type", JSON.stringify(this));
	}
...
function size(type)
{
	if (type.isArray)
	{
		return type.arrayCount * type.arrayOf(type.arrayOf());
	}
